...
def ab9_engage_gear(gear: str) -> dict:
    """Engage a gear: `N` (neutral), `1`..`7`, or `R`. Emits the byte-exact HID
    gear-state report (interrupt EP 0x83) when the gadget exposes an f_hid
    function (setup_ab9_gadget.sh --hid → /dev/hidg0), so SimHub / DirectInput
    sees the shift; also snaps the CDC analog X/Y to an approximate quadrant for
    tools that read D7/D8.
    """
    if _sim is None:
        return _no_sim()
    ...
    report = _sim.emit_hid_gear(key)
    return {'gear': key, 'shifter_x': x, 'shifter_y': y,
            'hid_attached': _sim._hid_fh is not None,
            'hid_report': report.hex()}
