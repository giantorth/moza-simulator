...
# ── AB9 HID gear-state report (interrupt EP 0x83) ───────────────────────────
# Decoded byte-exact from usb-capture/AB9/{all_gears,1-N}.pcapng (real AB9,
# USB 0x346e:0x1000, device 23). 34-byte interrupt-IN report:
#   [0]      report id = 0x01
#   [1..16]  8 x uint16-LE axes, centre 0x7FFF (wire 'ff 7f')
#   [17]     hat low-nibble (0x08 = centred) + gear buttons (bits 4-7)
#   [18]     gear buttons (continued, bits 0-7)
#   [19..33] reserved (zero)
# Each H-pattern gear engages exactly one button bit; neutral engages none.
AB9_HID_REPORT_LEN = 34
_AB9_AXIS_CENTRE = 0x7FFF
# gear label -> (byte17 OR-mask, byte18 OR-mask). byte17 always carries 0x08
# (hat centred). 1-6 confirmed (1 via 1-N.pcapng, 2-6 via all_gears.pcapng);
# 7 and R take the next free byte18 bits and are flagged until a 7-speed /
# reverse capture confirms them.
AB9_GEAR_BITS: Dict[str, Tuple[int, int]] = {
    'N': (0x00, 0x00),
    '1': (0x20, 0x00),   # byte17 bit5
    '2': (0x40, 0x00),   # byte17 bit6
    '3': (0x80, 0x00),   # byte17 bit7
    '4': (0x00, 0x01),   # byte18 bit0
    '5': (0x00, 0x02),   # byte18 bit1
    '6': (0x10, 0x00),   # byte17 bit4
    '7': (0x00, 0x04),   # byte18 bit2 (inferred)
    'R': (0x00, 0x08),   # byte18 bit3 (inferred)
}


def build_ab9_hid_report(gear: str = 'N') -> bytes:
    """Build the 34-byte AB9 HID gear-state report for a gear label (N, 1..7, R).
    Byte-exact against the captured reports — axes centred, one gear button bit
    set."""
    b17, b18 = AB9_GEAR_BITS.get(str(gear).upper(), (0x00, 0x00))
    rep = bytearray(AB9_HID_REPORT_LEN)
    rep[0] = 0x01
    centre = _AB9_AXIS_CENTRE.to_bytes(2, 'little')
    for i in range(8):
        rep[1 + i * 2:3 + i * 2] = centre
    rep[17] = 0x08 | b17
    rep[18] = b18
    return bytes(rep)


# USB HID report descriptor for the f_hid gadget function: a joystick with
# report ID 1, 8 x 16-bit unsigned axes, 16 buttons (the gear bits land here in
# bytes 17-18), and 15 bytes of constant padding -> a 34-byte input report
# matching build_ab9_hid_report(). The real AB9 report descriptor was not in the
# captures (no enumeration captured), so this is reconstructed to produce the
# captured report layout; DirectInput sees the gear bits as buttons.
AB9_HID_REPORT_DESCRIPTOR = bytes([
...
class Ab9Simulator:
    ...
    def __init__(self, identity: Optional[Dict[str, object]] = None,
        ...
        # HID gear-state (interrupt EP 0x83). `gear` is the current H-pattern
        # label; `_hid_fh` is the /dev/hidg<N> file when the gadget exposes an
        # f_hid function. Gear changes write a fresh 34-byte report.
        self.gear: str = 'N'
        self._hid_fh = None

    # ── HID gear-state emission ─────────────────────────────────────────────
    def open_hid(self, path: str) -> bool:
        """Open the gadget's HID endpoint (/dev/hidg<N>) for gear reports.
        Returns False if it can't be opened (no f_hid function present)."""
        try:
            self._hid_fh = open(path, 'wb', buffering=0)
            self.emit_hid_gear(self.gear)   # prime with the current gear
            return True
        except OSError:
            self._hid_fh = None
            return False

    def emit_hid_gear(self, gear: str) -> bytes:
        """Set the current gear and, if the HID endpoint is open, write the
        34-byte report. Returns the report bytes (also used by the self-test)."""
        self.gear = str(gear).upper()
        report = build_ab9_hid_report(self.gear)
        if self._hid_fh is not None:
            try:
                self._hid_fh.write(report)
            except OSError:
                self._hid_fh = None
        return report
    ...
    def snapshot(self) -> Dict[str, object]:
        ...
        return {
            ...
            'gear': self.gear,
            'hid_attached': self._hid_fh is not None,
...
def cmd_live(port: str) -> int:
    ...
    sim = Ab9Simulator()
    # Attach the HID gear-state endpoint if the gadget exposes an f_hid function
    # (setup_ab9_gadget.sh --hid). Best-effort: absent on a CDC-ACM-only gadget.
    import os as _os
    hid_path = _os.environ.get('AB9_HID_DEV', '/dev/hidg0')
    if _os.path.exists(hid_path) and sim.open_hid(hid_path):
        print(f'[HID gear reports on {hid_path} — gear={sim.gear}]', file=sys.stderr)
        log_fh.write(f'# HID gear-state attached on {hid_path}\n')
    else:
        print(f'[No HID endpoint ({hid_path}); gear reports disabled '
              f'(CDC-ACM-only gadget)]', file=sys.stderr)
...
def cmd_self_test() -> int:
    ...
    # HID gear-state reports — byte-exact against the captured reports from
    # usb-capture/AB9/{1-N,all_gears}.pcapng (real AB9 device 23, EP 0x83).
    hid_want = {
        'N': '01ff7fff7fff7fff7fff7fff7fff7fff7f0800000000000000000000000000000000',
        '1': '01ff7fff7fff7fff7fff7fff7fff7fff7f2800000000000000000000000000000000',
        '2': '01ff7fff7fff7fff7fff7fff7fff7fff7f4800000000000000000000000000000000',
        '3': '01ff7fff7fff7fff7fff7fff7fff7fff7f8800000000000000000000000000000000',
        '4': '01ff7fff7fff7fff7fff7fff7fff7fff7f0801000000000000000000000000000000',
        '5': '01ff7fff7fff7fff7fff7fff7fff7fff7f0802000000000000000000000000000000',
        '6': '01ff7fff7fff7fff7fff7fff7fff7fff7f1800000000000000000000000000000000',
    }
    for gear, want in hid_want.items():
        got = build_ab9_hid_report(gear).hex()
        if got != want:
            failures.append(f'HID report gear {gear}: want {want} got {got}')
    if len(AB9_HID_REPORT_DESCRIPTOR) < 40:
        failures.append('HID report descriptor looks too short')
...
def main() -> int:
    ...
    parser.add_argument('--hid-descriptor', action='store_true',
                        help='Write the AB9 HID report descriptor (binary) to stdout '
                             'for the f_hid gadget function (setup_ab9_gadget.sh --hid).')
    args = parser.parse_args()

    if args.hid_descriptor:
        sys.stdout.buffer.write(AB9_HID_REPORT_DESCRIPTOR)
        return 0
    if args.self_test:
