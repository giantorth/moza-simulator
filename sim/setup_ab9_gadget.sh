...
# --hid adds the f_hid gear-state interface (EP 0x83, 34-byte reports), matching
# the real AB9 composite. Default stays CDC-ACM-only for back-compat. With --hid,
# ab9_sim opens /dev/hidg<N> and ab9_engage_gear drives DirectInput-visible gears.
WANT_HID=0
for arg in "$@"; do
    [[ "$arg" == "--hid" ]] && WANT_HID=1
done
...
# Optional HID gear-state interface (real AB9 has it on MI_02 / EP 0x83). Linked
# AFTER acm so interface numbering matches (CDC on MI_00/01, HID on MI_02). The
# report descriptor + 34-byte report layout come from ab9_sim (captured-exact).
if [[ "$WANT_HID" == "1" ]]; then
    mkdir -p "$GADGET/functions/hid.usb0"
    echo 0  > "$GADGET/functions/hid.usb0/protocol"      # 0 = none (joystick)
    echo 0  > "$GADGET/functions/hid.usb0/subclass"      # 0 = no boot interface
    echo 34 > "$GADGET/functions/hid.usb0/report_length"
    if ! python3 "$(dirname "$0")/ab9_sim.py" --hid-descriptor \
            > "$GADGET/functions/hid.usb0/report_desc"; then
        echo "Failed to write HID report descriptor." >&2
        exit 1
    fi
    ln -sf "$GADGET/functions/hid.usb0" "$GADGET/configs/c.1/"
    echo "[+] HID gear-state interface enabled (34-byte reports) -> /dev/hidg<N>"
fi
